...
var copyDesiredStateToReportedStateInShadow=false;
if(USE_PHYSICAL_DEVICE =='true')
{
    log("***********************entered if ***********");
copyDesiredStateToReportedStateInShadow = false;
}
else{
    log("**************************entered else");
copyDesiredStateToReportedStateInShadow=true;
}
log("***********************copyDesiredStateToReportedStateInShadow***********");
log( typeof copyDesiredStateToReportedStateInShadow);
log(copyDesiredStateToReportedStateInShadow);

log("***********************USE_PHYSICAL_DEVICE***********");
log(typeof USE_PHYSICAL_DEVICE);
log(USE_PHYSICAL_DEVICE);
    ...
    try {
        ...
        // we do anything with it: 
            ...
            if (namespace === 'Alexa.PowerController') {
                let response = await handlePowerController(request, context, endpoint);
                return sendResponse(response.get());
            }
...
async function handleReportState(request, context, endpoint) {
    ...
    alexaResponse.addContextProperty({
      ...
      alexaResponse.addContextProperty({
        ...
        value:{
                hue: currentState.color.hue, 
                saturation: currentState.color.saturation,
                brightness: currentState.color.brightness
        }
    });
     alexaResponse.addContextProperty({
...
async function handlePowerController(request, context, endpoint) {
    /*  This function handles all requests that Alexa identifies as being a 
        "ThermostatController" directive, such as:
          - Turn device to cool mode
          - Turn device to heat mode
...
async function handleColorController(request, context, endpoint) {
    /*  This function handles all requests that Alexa identifies as being a 
        "ThermostatController" directive, such as:
          - Turn device to cool mode
          - Turn device to heat mode
    ...
    if (requestMethod === 'SetBrightness') {
        let shadowState = {
            state: {
                desired: {
                    brightness: {
                       value: payload.brightness.value
                    }
                }
            }
